            ...
            <div className="w-full overflow-x-auto">
                <table className="w-full text-sm text-left rtl:text-right text-gray-500">
                    <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                        <tr>
                            <th className="px-2 md:px-6 py-3">Transaction</th>
                            <th className="px-2 md:px-6 py-3">Amount</th>
                            <th className="px-2 md:px-6 py-3">Status</th>
                            <th className="px-2 md:px-6 py-3 max-md:hidden">Date</th>
                            <th className="px-2 md:px-6 py-3 max-lg:hidden">Channel</th>
                            <th className="px-2 md:px-6 py-3 max-md:hidden">Category</th>
                        </tr>
                    </thead>
                    <tbody>
                        {currentTransactions.map((t) => {
                            ...
                            return (
                                <tr key={t.id} className="bg-white border-b hover:bg-gray-50">
                                    <td className="px-2 md:px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                        <div className="flex items-center gap-2">
                                            {t.name}
                                        </div>
                                    </td>
                                    <td className={`px-2 md:px-6 py-4 font-semibold ${amountClass}`}>
                                    ...
                                    <td className="px-2 md:px-6 py-4 max-md:hidden">{t.date}</td>
                                    <td className="px-2 md:px-6 py-4 capitalize max-lg:hidden">{t.paymentChannel.replace('_', ' ')}</td>
                                    <td className="px-2 md:px-6 py-4 max-md:hidden">
